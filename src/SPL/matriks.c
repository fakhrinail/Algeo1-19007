...
/* Melakukan assignment MHsl  MIn */
{
   ...
   for (i=BrsMin; i<=GetLastIdxBrs(*MHsl); i++){
      for (j=KolMin; j<=GetLastIdxKol(*MHsl); j++){
...
/* Mengirim hasil perkalian matriks: salinan M1 * M2 */
{
   ...
   ElType ElmtTemp;
   for (i=BrsMin; i<=GetLastIdxBrs(MNew); i++){
      for (j=KolMin; j<=GetLastIdxKol(MNew); j++){
         int brsM1=i;
         int kolM1=0;
         int brsM2=0;
         int kolM2=j;
         ElmtTemp = 0;
         while (kolM1<=GetLastIdxKol(M1)){
            ElmtTemp += Elmt(M1,brsM1,kolM1) * Elmt(M2,brsM2,kolM2);
            kolM1+=1;
            brsM2+=1;
         }
         Elmt(MNew,i,j) = ElmtTemp;
      }
   }
   return MNew;   
} 
...
{
   if (NBElmt(M1)==NBElmt(M2)){
      boolean tidakSama = false;
      indeks i=BrsMin;
      while (i<=GetLastIdxBrs(M1) && !tidakSama){
         indeks j=KolMin;
         while (j<=GetLastIdxKol(M1) && !tidakSama){
            if (Elmt(M1,i,j) == Elmt(M2,i,j)){
               j+=1;
            }else{
               tidakSama = true;
   ...
   hanya maksimal 5% dari memori matriks yang efektif bukan bernilai 0 */ 
{
   ...
   return 0.05 >= (counter/(double)NBElmt(M));
}
...
/* Menghitung nilai determinan M */
{
   if (NBrsEff(M)==2 && NKolEff(M)==2){
      ...
      for (j=KolMin;j<=GetLastIdxKol(M);j++){  
         ...
         indeks k,l;
         for (k=BrsMin+1;k<=GetLastIdxBrs(M);k++){
            for (l=KolMin;l<=GetLastIdxKol(M);l++){
               if (l<j){
                  Elmt(MTemp,k-1,l) = Elmt(M,k,l);
               }else if (l>j){
                  Elmt(MTemp,k-1,l-1) = Elmt(M,k,l);
               }
            }
         }
         float isneg = (j%2==0) ? 1:-1;
         determinan += isneg * Elmt(M,i,j) * Determinan(MTemp);
      }
      return determinan;
   }
}
